%\chapter{Theory} 
\chapter{Motivation for studying \decay{\Bp}{\Dsp\Kp\Km} final states} 
\label{ch:theory}
...
%Although the weak interaction dictates the core of the process, the contributions from the strong force are unavoidable. 
...
\end{equation}
%Therefore, the extension of the mixing matrix not only allows all three generations to be considered, but also increases the number of terms that parametrise the mixing matrix from one to four. 
...
%%%%% Add description of type of CP violation
In the standard model \CP violation can manifest itself in three different ways: 
\begin{enumerate}
...
%%%%% Adding description of the unitarity triangle




The first observation of \CP violation occurred in 1964 by V. Fitch and J. Cronin in the decay of neutral kaons~\cite{PhysRevLett.13.138}. The kaon flavour eigenstates \Kz (\dquark\squarkbar) and \Kzb (\dquarkbar\squark) mix into the physically observed \KL and \KS particles with long and short lifetimes respectively. Under the assumption of \CP conservation these would correspond to the states $(\dquark\squarkbar-\dquarkbar\squark)/\sqrt{2}$ and $(\dquark\squarkbar+\dquarkbar\squark)/\sqrt{2}$ respectively. 
A beam of long-lived neutral kaons was produced and the decay to $2\pi$ was observed. Neutral kaons were known to decay to $2\pi$ and $3\pi$ final states. The $2\pi$ state is a $+1$ eigenstate of the \CP operation, whilst the $3\pi$ is a $-1$ eigenstate. If the long-lived and short-lived neutral kaon eigenstates were also \CP eigenstates then only the $\decay{\KS}{\pi\pi}$ and $\decay{\KL}{\pi\pi\piz}$ decays would be allowed. The observation of \decay{\KL}{\pi\pi} decays demonstrated that the physical kaon states do not conserve \CP symmetry. This \CP asymmetry is explained by the CKM matrix. 
...
\begin{figure}[!h]
    ...
    \end{subfigure}
    \caption{Two-body weak decay topologies of \bquark-mesons. The \W-boson exchange and penguin-annihilation topologies only contribute to neutral \B meson decays, whilst the annihilation topology only contributes to charged \B mesons. Colour-favoured and colour-suppressed refer to the constraints that are imposed by requiring the final state mesons to be colourless.}
...
Pure annihilation topology decays are processes that only receive contributions from annihilation decay diagrams. By measuring the rates of these decays the magnitude of annihilation processes can be isolated.
%Pure annihilation topology decays are an interesting subset of processes in which (at lowest order) only annihilation decay diagrams contribute. These are of particular interest because this allows the magnitudes of these processes to be isolated. 
Additionally, these decays provide a suitable forum in which to search for new physics; enhanced or decreased branching fractions and non-zero \CP violations are symptomatic of additional competing processes causing interference.  

Typically, \emph{pure} hadronic annihilation processes are characterised a different set of quark flavours in the initial and final state, such that no quark can act as a spectator. This implies that the initial state must have completely annihilated into a mediator for this process to occur.   
...
% \item $\omega - \phi$ mixing: ~\cite{Okubo:1963fa,PhysRevD.79.074006}.
% \end{itemize}}
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The decay \decay{\Bp}{\Dsp\Kp\Km} can therefore be characterised by the two-dimensional space defined by $m^{2}(\Dsp\Km)$ and $m^{2}(\Kp\Km)$. Resonances could contribute to \decay{\Bp}{\Dsp\Kp\Km} decays, for example as \decay{\Bp}{\Dsp(\decay{X}{\Kp\Km})} or \decay{\Bp}{\Kp(\decay{Y}{\Dsp\Km})}. The distribution of decays in the Dalitz plot are indicative of the spin and mass of contributing resonances.
