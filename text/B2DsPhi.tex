...
%in ways that could not be easily accounted for if the modes were combined in a single data set. 
 ...
 %This leads to the background from combinations of unrelated tracks having a larger relative contribution.
...
The fraction of decays expected in each $m(\Kp\Km)$ and $\cos\theta_{K}$ category for the different models considered are tabulated in Table~\ref{table:DsKK_rescfracs}. 
%These are calculated by counting the numbers of entries in the corresponding ranges delineated by the vertical lines in Figs.~\ref{fig:DsKK_model_phi1020}-\ref{fig:DsKK_model_a21320}. 
For reference, the $\phi(1020)$ fractions are included for both the fully simulated decays and those generated with \laurapp. The maximum difference between these fractions is included as a source of systematic uncertainty (Sec.~\ref{sec:B2DsPhi_systuncertainy}).
...
% The two CB function are allowed have different widths, $\sigma_{1}$ and $\sigma_{2}$, but the ratio $\sigma_{1}/\sigma_{2}$ is fixed from the fits to simulations, as is $f_{\sigma}$ that determines the fractional contribution of the narrower CB function ($\sigma_{1}<\sigma_{2}$). 
However, an extra constraint is added to the signal and normalisation DCB functions with respect to the previous configuration. As the number of signal candidates is likely to be small, the relative width of the narrower signal and normalisation CB functions, $\sigma_{1}(\Dsp\phi) / \sigma_{1}(\Dsp\Dzb)$, is also fixed to values obtained from the fits to simulations. 
...
%       \hline
%       Missed            & Helicity  & Angular         & $g(\mu,a,b)$                    & $a$ (\mevcc) & $b$ (\mevcc)  \\
%       particle          & state     & distribution    &                                 &              &                \\
%       \hline
%       \piz              & 010       & $\cos^{2}{\theta_{K}}$   & $\left(\mu - \frac{a+b}{2}\right)^{2}$  & 5026.8       & 5124.8         \\
%       \piz              & 101       & $\sin^{2}{\theta_{K}}$   & $-(\mu - a)(\mu-b)$                               & 5026.8       & 5124.8         \\
%       \Pgamma           & 010       & $\sin^{2}{\theta_{K}}$   & $-(\mu - a)(\mu-b)$                               & 4936.4       & 5220.6         \\
%       \Pgamma           & 101       & $1+\cos^{2}{\theta_{K}}$ & $(\mu - \frac{a+b}{2})^{2} +(\frac{a+b}{2})^{2} $ & 4936.4       & 5220.6         \\
%       \hline
%    \end{tabular}
...
This includes: a single combinatorial background slope parameter, $c$; the mean \Bp mass, $\mu$; the partially reconstructed background mass offset, $\delta$; the relative height of the double peaked \decay{\Bp}{\Dssp\phiz}, \decay{\Bp}{\Dssp\Dzb} and \decay{\Bp}{\Dsp\Dstarzb} decays, $\xi$; and the narrower CB width parameter $\sigma_{1}$ (for each \Dsp mode). 
% \begin{itemize}
...
%This results in small efficiencies around 2\% for each mode. 
...
The efficiency of the PIDK requirements are determined using \pidcalib~\cite{PIDCalib}. 
% This uses calibrations samples for the different particle species to determine the fraction of candidates passing the various PID variable requirements. 
...
%The calibration samples for both \Kp and \pip mesons are collected from a sample of \decay{\Dstarp}{(\decay{\Dz}{\Kp\pim})\pip} decays, using the decay products of the \Dz decay. 
...
The simultaneous fit effectively measures a single value of $R$ for all \Dsp decay mode categories. 
%From an ensemble of pseudoexperiments, $R$ is distributed normally. 
