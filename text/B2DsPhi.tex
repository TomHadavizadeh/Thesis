...
{\color{Blue}
The $\B^+ \to \Dsp^{*} \phi$ decay contributes a partially reconstructed background to the $\Bu \to \Dsp \phi$ signal mode.
...
\begin{description}
\item \textbf{\decay{\Bsb}{\Dsp\Km\Kstarz}:} 
...
f(m|c) = e^{-m\times c},
\end{equation}
where the slope is controlled by the parameter $c$ and $m$ is the observable \Bp meson invariant mass. 
