...
\subsection{\boldmath{$\decay{\Bp}{\Dsp\Kp\Km}$} model assumption}
...
The $\text{CL}_{\text{S}}$ distribution is shown in Fig.~\ref{fig:B2DsPhi_limit_CLS} which included the $1\sigma$ bands in green and $2\sigma$ bands in yellow.
...
% As no significant yield of $\B \to \Ds \phiz$ events are observed, an upper limit is set on the branching fraction using the CLs method implemented within the \texttt{RooStats} package. This method tests the hypothesis of a signal plus background model against a background only model where the branching fraction is fixed to zero. All other floating variables are considered as nuisance parameters. The p-value distribution is shown in Fig.~\ref{fig:limit} which included the 1$\sigma$ bands in green and 2$\sigma$ bands in yellow. The 95\% upper limit is determined as the point where the CLs p-values fall below 5\% as illustrated by the red line. This results in a upper limit on the branching fraction of:

% \begin{equation}
% Br(\B \to \Ds \phi) < 4.82 \times 10^{-7} (95\%)
% \label{eq:upperlimit}
% \end{equation}
