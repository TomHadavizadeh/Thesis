...
\begin{description}
\item \textbf{PID:} the efficiency for the particle identification requirements is calculated by correcting the PID variables in simulation using a package called \texttt{PIDCorr} and \texttt{Meerkat} {\color{Red}Cite}. This differs from the method used to determine the PID efficiencies in the search for \decay{\Bp}{\Dsp\phiz} decays as the phase-space variations must be considered.
\end{description}
...
\begin{figure}[!h]
   ...
   \begin{subfigure}[t]{0.4\textwidth}
      ...
      \caption{2012}
   \end{subfigure}
   \caption{PID efficiency variation as a function $m(\Kp\Km)$.}
   \label{fig:B2DsKK_PID_eff}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
