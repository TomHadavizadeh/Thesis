...
The background samples are comprised of the sideband regions of the relevant ranges in Fig.~\ref{fig:mvatrainingsamples}. These ranges are defined by $|m(\Kp\Km)-m(\phiz)| > 10 \mevcc$ and $|m(h^{+}h^{-}h^{+})-m(\Dsp)| > 30\mevcc$ for the \phiz and \Dsp MVAs respectively. 
The efficacy of three methods are compared and the most effective selected. The three methods are derivatives of Boosted Decision Trees (BDTs)~\cite{Breiman}.
...
\hline
Run      & \Dsp decay mode               & \Dsp requirement & \phiz/\Dzb requirement \\ 
\hline
...
{\color{Red}
\begin{itemize}
\item MVA make requirements on \Dsp and \phiz candidates, not combination
\item possible to tighten \Bp selection further
\item \Bp MVA not required
\item IP shows two types of BGs:
