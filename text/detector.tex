...
The first stage, \hltone, reduces the rate of events by matching \lone deposits to tracks in the \velo and T-stations. The software algorithm performs partial pattern recognition to match the charged particle \lone deposits to track candidates and or to ensure that \Pgamma or \piz \lone candidates are not associated to tracks. This part of the trigger reduces the \lone output rate to around 43\,kHz.

The second stage, \hlttwo, performs a full event reconstruction  
...
Particles are reconstructed offline using information from the various sub-detectors. 
...
{\color{Red}
\begin{itemize}
\item PArticle Identification: hadrons
\item muons
\item electrons 
\end{itemize}
}
...
\begin{itemize}
\item\textbf{Photons:} Isolated \ecal clusters with an corresponding cluster in the \presh are used to create photon candidates. The energy is determined from the total energy in the \ecal and \presh. 
\item \textbf{Neutral pions:} These mesons decay dominantly to two photons, however the reconstruction depends on the transverse momentum of the \piz. For low \pt \piz meson decays the two photons are well separated (\emph{resolved}) leading to two photon deposits. For higher \pt \piz mesons the two photon deposits overlap (\emph{merged}) leading to a single \ecal cluster. 
\end{itemize}
