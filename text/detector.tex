...
\label{eq:hlt_mcorr}
\end{equation}   
where $\pt'_{\text{miss}}$ is the missing momentum transverse to the flight direction of the particle, defined by the vector between the PV and 2-, 3-, or 4-body vertex. A typical distribution of the corrected mass is shown for $\decay{\B}{K\pi\pi}$ decays in Fig.~\ref{fig:Dec_hlt_mcorr}.
Requirements are made to $m_{\text{corr}}$ to ensure the vertex is consistent with a \bquark-hadron decay.
...
\begin{figure}[!h]
    ...
    \includegraphics[width=0.5\textwidth]{figs/Detector/InclusiveHlt2Lines_justtwo.pdf}

    \caption{The invariant mass of two of the three final state particles in $\decay{\B}{K\pi\pi}$ decays using the sum of four momenta (shaded) and the corrected mass as defined in Eq.~\ref{eq:hlt_mcorr} (line), from Ref.~\cite{Williams:2011aza}.}
